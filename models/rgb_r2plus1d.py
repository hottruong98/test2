...
class rgb_r2plus1d_32f_34(nn.Module):
    def __init__(self, num_classes, length, model_path=''):
        super(rgb_r2plus1d_32f_34, self).__init__()
        self.num_classes = num_classes
        self.dp = nn.Dropout(p=0.8)
        self.avgpool = nn.AdaptiveAvgPool3d(output_size=(1, 1, 1))
        self.features = nn.Sequential(* list(r2plus1d_34_32_ig65m(359, pretrained=True, progress=True).children())[:-2])
...
class rgb_r2plus1d_32f_34_bert10(nn.Module):
    ...
    def forward(self, x):
        ...
        output, mask_sample = self.bert(x)
        cls_out = output[:, 0, :]
        sequence_out = output[:, 1:, :]
        norm = sequence_out.norm(p=2, dim=-1, keepdim=True)
        sequence_out = sequence_out.div(norm)
        output = self.dp(cls_out)
        x = self.fc_action(output)
        return x, input_vectors, sequence_out, mask_sample